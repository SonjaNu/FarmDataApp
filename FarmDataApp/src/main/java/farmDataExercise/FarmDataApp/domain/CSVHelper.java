...
public class CSVHelper {
	...
	static String[] HEADERs = { "Location", "Datetime", "SensorType", "Value" };
	...
	// CSVParser object is created from BufferedReader and InputStream --> calls a predefined method getRecords(), 
    // which returns the content present in the csv file in the form of record
	public static List<Farm> csvToFarms(InputStream is) {
		try (BufferedReader fileReader = new BufferedReader(new InputStreamReader(is, "UTF-8"));
			...
			for (CSVRecord csvRecord : csvRecords) {
				...
				if (!csvRecord.get("Location").equals("") && !csvRecord.get("Datetime").equals("")
						&& !csvRecord.get("SensorType").equals("") && !csvRecord.get("Value").equals("")
						&& csvRecord.get("Value").matches("[0-9.-]*")) {
      	  
					Farm farm = new Farm(csvRecord.get("Location"), OffsetDateTime.parse(csvRecord.get("Datetime")),
							csvRecord.get("SensorType"), Double.parseDouble(csvRecord.get("Value")));

					// Validation with given rules
					if (farm.getSensorType().equals("pH") && (farm.getValue() >= 0.0 && farm.getValue() <= 14.0)) {
						farmList.add(farm);
					} else if (farm.getSensorType().equals("temperature")
							&& (farm.getValue() >= -50.0 && farm.getValue() <= 100.0)) {
						farmList.add(farm);
					} else if (farm.getSensorType().equals("rainFall")
							&& (farm.getValue() >= 0.0 && farm.getValue() <= 500.0)) {
						farmList.add(farm);
					}
				}
	...
	// Public class ByteArrayInputStream implements an output stream in,
		...
		try (ByteArrayOutputStream out = new ByteArrayOutputStream();
			...
			for (Farm farm : farmList) { // iterate the list
				List<String> data = Arrays.asList( // and store values in the form of a String List
						farm.getLocation(), String.valueOf(farm.getDatetime()), farm.getSensorType(),
